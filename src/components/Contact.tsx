                ...
                <div className="bg-white rounded-[44px] p-8 lg:p-14 shadow-[0_32px_80px_-20px_rgba(0,0,0,0.15)] border border-gray-100">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
                        ...
                        <div className="lg:col-span-5 flex flex-col gap-8">
                            <div>
                                <span className="inline-block px-4 py-1.5 rounded-full bg-[#2a7dc9]/10 text-[#2a7dc9] text-xs font-black tracking-widest uppercase mb-6 border border-[#2a7dc9]/20">
                                    Contact Us
                                </span>
                                ...
                                <p className="text-gray-500 text-lg">
                                    ...
                                    <span className="text-brand-navy font-medium">Wholesale opportunities also available.</span>
                                </p>
                            </div>
                        ...
                        <div className="lg:col-span-7">
                            {isSubmitSuccessful ? (
                                <div className="py-12 text-center text-brand-navy font-black bg-brand-amber/10 rounded-3xl border border-brand-amber/20 uppercase font-outfit tracking-widest">
                                ...
                                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <input {...register("name")} placeholder="Full Name" className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-[#2a7dc9] focus:ring-0 transition-all text-gray-800 placeholder:text-gray-400 font-medium" />
                                            {errors.name && <p className="text-red-500 text-xs px-2">{errors.name.message}</p>}
                                        </div>
                                        <div className="space-y-1">
                                            <input {...register("email")} placeholder="Email Address" className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-[#2a7dc9] focus:ring-0 transition-all text-gray-800 placeholder:text-gray-400 font-medium" />
                                            {errors.email && <p className="text-red-500 text-xs px-2">{errors.email.message}</p>}
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                        <textarea {...register("message")} placeholder="How can we help?" rows={3} className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-[#2a7dc9] focus:ring-0 transition-all text-gray-800 placeholder:text-gray-400 font-medium resize-none" />
                                        {errors.message && <p className="text-red-500 text-xs px-2">{errors.message.message}</p>}
                                    </div>
                                    <button
                                        type="submit"
                                        disabled={isSubmitting}
                                        className="w-full py-4 bg-[#2a7dc9] text-white font-extrabold rounded-full shadow-lg shadow-[#2a7dc9]/25 hover:bg-[#2176c1] hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-200 text-sm tracking-wide mt-2 cursor-pointer"
                                    >
